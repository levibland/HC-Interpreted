...
{
    holyc_scope_T* holyc_scope = calloc(1, sizeof(struct holyc_SCOPE_STRUCT));
    holyc_scope->variable_definitions = init_dynamic_list(sizeof(struct AST_STRUCT*));
    holyc_scope->function_definitions = init_dynamic_list(sizeof(struct AST_STRUCT*));
    holyc_scope->global = global;
...
{
    if (holyc_scope->variable_definitions)
    {
        for (int i = 0; i < holyc_scope->variable_definitions->size; i++)
        {
            AST_T* vardef = (AST_T*) holyc_scope->variable_definitions->items[i];
        ...
        free(holyc_scope->variable_definitions->items);
        free(holyc_scope->variable_definitions);
        holyc_scope->variable_definitions = (void*)0;
    }

    if (holyc_scope->function_definitions)
    {
        for (int i = 0; i < holyc_scope->function_definitions->size; i++)
        {
            AST_T* fdef = (AST_T*) holyc_scope->function_definitions->items[i];
            ...
            if (fdef->function_definition_body)
            {
                ...
                {
                    if ((holyc_scope_T*)fdef->function_definition_body->scope != holyc_scope)
                        holyc_scope_free((holyc_scope_T*)fdef->function_definition_body->scope);
                }*/
        ...
        free(holyc_scope->function_definitions->items);
        free(holyc_scope->function_definitions);
        holyc_scope->function_definitions = (void*) 0;
    }
