...
void ast_free(AST_T* ast)
{
    ...
    /*if (ast->scope)
    {
        holyc_scope_T* scope = (holyc_scope_T*) ast->scope;

        if (!scope->global)
        {
            holyc_scope_free(scope);
            scope = (void*) 0;
            ast->scope = (void*)0;
        }
    }*/
...
static AST_T* ast_r_scope(AST_T* ast, struct holyc_scope_T* scope)
{
    ast->scope = scope;
    return ast;
}
...
char* ast_to_string(AST_T* ast)
{
    switch (ast->type)
    {
        ...
        case AST_CHAR: return holyc_char_to_string(ast->char_value); break;
...
char* ast_list_access_to_string(AST_T* ast)
{
    ...
    return holyc_init_str("[ list_access ]");
}
